...
import { buildSiteCanonical, listMarketSitemapEntries } from "@/lib/seo/site-page-seo";
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const marketRoutes: MetadataRoute.Sitemap = listMarketSitemapEntries().map((entry) => ({
    url: buildSiteCanonical(entry.path),
    lastModified: now,
  ...
  if (!activeSymbols?.length) {
    return [...marketRoutes, ...wealthRoutes];
  }
  ...
  return [...marketRoutes, ...wealthRoutes, ...stockRoutes];
}
