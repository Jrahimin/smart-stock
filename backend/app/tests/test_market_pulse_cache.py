...
def _pulse(greeting: str) -> MarketPulseRead:
...
def _summary(greeting: str, last_synced_at: datetime | None) -> MarketPulseSummaryRead:
...
def _service_with_redis(store: dict[str, dict]) -> MarketPulseService:
    ...
    market_data_service.get_market_freshness = AsyncMock(
        return_value=MagicMock(last_synced_at=datetime(2026, 7, 9, 10, 0, tzinfo=timezone.utc)),
    )
...
async def test_anonymous_summary_reads_and_writes_shared_cache() -> None:
    ...
    assert pulse_cache_key("summary", ExchangeCode.DSE) in store
...
async def test_display_name_bypasses_shared_cache_reads_and_writes() -> None:
    ...
    assert pulse_cache_key("summary", ExchangeCode.DSE) not in store
...
async def test_different_display_names_do_not_share_cached_personalized_output() -> None:
    store: dict[str, dict] = {}
    service = _service_with_redis(store)
    store[pulse_cache_key("response", ExchangeCode.DSE)] = _pulse("Good morning, Alex").model_dump(mode="json")
...
async def test_legacy_cached_summary_without_generation_is_treated_as_miss() -> None:
    store: dict[str, dict] = {}
    service = _service_with_redis(store)
    store[pulse_cache_key("summary", ExchangeCode.DSE)] = _summary(
        "Good morning",
        None,
    ...
    service._compute_market_pulse.assert_awaited_once()
    assert pulse_cache_key("summary", ExchangeCode.DSE) in store
    assert store[pulse_cache_key("summary", ExchangeCode.DSE)]["last_synced_at"] is not None
...
async def test_stale_pulse_response_cache_is_not_reused_when_rebuilding_summary() -> None:
    store: dict[str, dict] = {}
    service = _service_with_redis(store)
    store[pulse_cache_key("response", ExchangeCode.DSE)] = _pulse("Good morning, stale").model_copy(
        update={"last_synced_at": datetime(2026, 7, 8, 10, 0, tzinfo=timezone.utc)},
    ).model_dump(mode="json")
...
async def test_malformed_cached_summary_is_deleted_and_recomputed() -> None:
    store: dict[str, dict] = {}
    service = _service_with_redis(store)
    store[pulse_cache_key("summary", ExchangeCode.DSE)] = {"hero": "not-a-valid-summary"}
