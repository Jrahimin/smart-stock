...
Snapshots are created only after daily DSEX finalization, never by Pulse HTTP requests or intraday refreshes. Finalization reuses the already-calculated canonical universe for that `market_sync_id`; Pulse does not run a second full-universe decision calculation. Snapshots are keyed by exchange, session date, Pulse score, strategy, threshold, input-schema, and decision-taxonomy versions; they also retain candidate counts, an eligible-population fingerprint, source generation, and universe payload revision for audit.
